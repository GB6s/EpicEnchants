...
public class ItemGroup {
    ...
    public Set<String> getGroups(Set<Material> materials) {
        ...
        Set<Material> copy = new HashSet<>(materials);

        for (int i = 0; i < 5; i++) {
            getGroup(copy).ifPresent(group -> {
                groups.add(group.getName());
                copy.removeAll(getMaterials(group));
            });
        }

        groups.addAll(copy.stream().map(Material::toString).collect(Collectors.toSet()));
        return groups;
    }
